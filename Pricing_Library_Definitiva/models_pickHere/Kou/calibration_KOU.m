...
% Params to calibrate: [sigma, lambda, p, lambdaplus, lambdaminus]

% sigma:            volatility of the diffusion component
% lambda :          intensity of Poisson Process
% p:                probability of positive jumps
% lambdaplus:       law of positive jumps
% lambdaminus:      law of negative jumps

%par=lsqnonlin(@(x) priceKOU(x,S0,strike,r,maturity,pmkt),...
%  [0.3 30 0.5 4 4],[0.01 0 0 0 0], [1 100 1 20 20])
par=lsqnonlin(@(x) priceKOU_IV(x,S0,strike,r,maturity,pmkt),...
  [0.3 30 0.5 4 4],[0.01 0 0 0 0], [1 100 1 20 20])

[Error,PriceKOU]=priceKOU(par,S0,strike,r,maturity,pmkt);
...
figure
subplot(1,2,1);
plot(PriceKOU,'sm','Markersize', 7, 'Linewidth', 1.5)
hold on
plot(pmkt,'+b','Markersize', 7, 'Linewidth', 1.5)
legend('Calibrated Price (KOU)','Market Price')
title('Prices fitting'); grid on;

ImpVolatilityKOU = blsimpv(S0,strike, r, maturity, PriceKOU);

% Plot of Implied Volatilities
subplot(1,2,2);
plot(ImpVolatilityKOU,'sm','Markersize', 7, 'Linewidth', 1.5)
hold on
plot(ImpVolatilityMARKET,'+b','Markersize', 7, 'Linewidth', 1.5)
legend('Calibrated ImpVol (KOU)','Market ImpVol')
