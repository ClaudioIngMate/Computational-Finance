...
% Params to calibrate: [theta, csi, rho, eta, V0]
...
%par=lsqnonlin(@(x) priceHESTON(x,S0,strike,r,maturity,pmkt),...
%  [0.3 0.5 0 0.1 0.3],[0.01 0 -1 -2 0.01], [1 3 1 2 1])

par=lsqnonlin(@(x) priceHESTON_IV(x,S0,strike,r,maturity,pmkt),...
  [0.3 0.5 0 0.1 0.3],[0.01 0 -1 -2 0.01], [1 3 1 2 1])

[Error,PriceHESTON]=priceHESTON(par,S0,strike,r,maturity,pmkt);
...
figure
subplot(1,2,1);
plot(PriceHESTON,'sm','Markersize', 7, 'Linewidth', 1.5)
hold on
plot(pmkt,'+b','Markersize', 7, 'Linewidth', 1.5)
legend('Calibrated Price (HESTON)','Market Price')
title('Prices fitting'); grid on;

ImpVolatilityHESTON = blsimpv(S0,strike, r, maturity, PriceHESTON);

% Plot of Implied Volatilities
subplot(1,2,2);
plot(ImpVolatilityHESTON,'sm','Markersize', 7, 'Linewidth', 1.5)
hold on
plot(ImpVolatilityMARKET,'+b','Markersize', 7, 'Linewidth', 1.5)
legend('Calibrated ImpVol (HESTON)','Market ImpVol')
