...
%% Calibrating the Extended NIG

% Params to calibrate: [sigma, theta, k, sigmaGBM]

% sigma:        volatility of the diffusion component
% theta:        drift of the diffusion component
% k:            variance of the subordinator
% sigmaGBM:     volatility of the pure Brownian Motion (no time change)

% par=lsqnonlin(@(x) priceExtNIG(x,S0,strike,r,maturity,pmkt),...
%   [0.25 0.4 0.4 0.25],[0.01 -3 0 0.01], [1 3 5 1])

par=lsqnonlin(@(x) priceExtNIG_IV(x,S0,strike,r,maturity,pmkt),...
 [0.25 0.4 0.4 0.25],[0.01 -3 0 0.01], [1 3 5 1])

[Error,PriceExtNIG]=priceExtNIG(par,S0,strike,r,maturity,pmkt);
...
figure
subplot(1,2,1);
plot(PriceExtNIG,'sm','Markersize', 7, 'Linewidth', 1.5)
hold on
plot(pmkt,'+b','Markersize', 7, 'Linewidth', 1.5)
legend('Calibrated Price (ExtNIG)','Market Price')
title('Prices fitting'); grid on;

ImpVolatilityExtNIG = blsimpv(S0,strike, r, maturity, PriceExtNIG);

% Plot of Implied Volatilities
subplot(1,2,2);
plot(ImpVolatilityExtNIG,'sm','Markersize', 7, 'Linewidth', 1.5)
hold on
plot(ImpVolatilityMARKET,'+b','Markersize', 7, 'Linewidth', 1.5)
legend('Calibrated ImpVol (ExtNIG)','Market ImpVol')
