...
% par=lsqnonlin(@(x) priceNIG(x,S0,strike,r,maturity,pmkt),...
%   [0.3 0.4 0.4],[0.01 -2.5 0.01], [1 3 3])
par=lsqnonlin(@(x) priceNIG_IV(x,S0,strike,r,maturity,pmkt),...
 [0.3 0.4 0.4],[0.01 -2.5 0.01], [1 3 3])

[Error,PriceNIG]=priceNIG(par,S0,strike,r,maturity,pmkt);
...
figure
subplot(1,2,1);
plot(PriceNIG,'sm','Markersize', 7, 'Linewidth', 1.5)
hold on
plot(pmkt,'+b','Markersize', 7, 'Linewidth', 1.5)
legend('Calibrated Price (NIG)','Market Price')
title('Prices fitting'); grid on;

ImpVolatilityNIG = blsimpv(S0,strike, r, maturity, PriceNIG);

% Plot of Implied Volatilities
subplot(1,2,2);
plot(ImpVolatilityNIG,'sm','Markersize', 7, 'Linewidth', 1.5)
hold on
plot(ImpVolatilityMARKET,'+b','Markersize', 7, 'Linewidth', 1.5)
legend('Calibrated ImpVol (NIG)','Market ImpVol')
