...
% par=lsqnonlin(@(x) priceExtVG(x,S0,strike,r,maturity,pmkt),...
%   [0.25 0.4 0.4 0.25],[0.01 -3 0 0.01], [1 3 3 1])
par=lsqnonlin(@(x) priceExtVG_IV(x,S0,strike,r,maturity,pmkt),...
 [0.25 0.4 0.4 0.25],[0.01 -3 0 0.01], [1 3 3 1])

[Error,PriceExtVG]=priceExtVG(par,S0,strike,r,maturity,pmkt);
...
figure
subplot(1,2,1);
plot(PriceExtVG,'sm','Markersize', 7, 'Linewidth', 1.5)
hold on
plot(pmkt,'+b','Markersize', 7, 'Linewidth', 1.5)
legend('Calibrated Price (ExtVG)','Market Price')
title('Prices fitting'); grid on;

ImpVolatilityExtVG = blsimpv(S0,strike, r, maturity, PriceExtVG);

% Plot of Implied Volatilities
subplot(1,2,2);
plot(ImpVolatilityExtVG,'sm','Markersize', 7, 'Linewidth', 1.5)
hold on
plot(ImpVolatilityMARKET,'+b','Markersize', 7, 'Linewidth', 1.5)
legend('Calibrated ImpVol (ExtVG)','Market ImpVol')
