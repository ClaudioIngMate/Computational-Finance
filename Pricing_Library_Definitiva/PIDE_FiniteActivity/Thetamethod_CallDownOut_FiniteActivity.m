...
% exploiting Theta_Method + Operator Splitting (Implicit Euler for Barrier)

%% Contract params, spot and risk-free rate

r=0.001;  S0=100; %market params
T=1; K=105; D=85; d=log(D/S0);%contract params
...
nu=@(y) p.*lambda.*lambdaplus.*exp(-lambdaplus.*y).*(y>0) ...
         + (1-p).*lambda.*lambdaminus.*exp(-lambdaminus.*abs(y)).*(y<0); % Levy measure Kou
...
% At maturity
V=max(S0*exp(x')-K,0).*(x'>d); %%%%%%%%%%% Payoff at maturity (Down&Out Call)
BC=zeros(N+1,1); %%%%%%%%%%% Boundary condition on xmin (Down&Out Call)

for j=M:-1:1
    % known t_j --> unknown t_{j-1}
    BC(end)=S0*exp(xmax)-K*exp(-r*(T-(j-1)*dt)); %%%%%%%%% Boundary condition xmax for Call
...
index=(y<=x(1));
v(index)=0; %%%%%%%%%%%%%%%% Boundary condition on xMIN for Barrier
%% y>= xmax    BOUNDARY CONDITION on xMAX
index=(y>=x(end));
v(index)=S0*exp(y(index))-K*disc; %%%%%%%%%% Boundary condition xMAX ( Call)
%% xmin <y <xmax
