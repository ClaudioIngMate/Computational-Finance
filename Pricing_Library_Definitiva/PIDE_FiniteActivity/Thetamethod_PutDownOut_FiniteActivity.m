...
% At maturity
V=max(K-S0*exp(x'),0).*(x'>d); %%%%%%% Payoff at maturity (Down&Out Put)
BC=zeros(N+1,1); %%%%%%%%%%%%%%% Boundary conditions are always 0 for Down&Out Put

for j=M:-1:1
    % known t_j --> unknown t_{j-1}
    % BC always 0 for Down&Out Put
...
index=(y<=x(1));
v(index)=0; %%%%%%%%%%%%%%%% BC always 0 for Down&Out Put
