...
% Price an American Put Option in the Merton model
...
% w.r.t. B&S PDE, I add lambda and alpha
...
% At maturity
V=max(K-S0*exp(x'),0); %% Payoff at maturity
BC=zeros(N+1,1);
for j=M:-1:1
    % known t_j --> unknown t_{j-1}
    BC(1)=K-S0*exp(xmax);
    % Compute the Integral at time t_j
    I=levy_integral2(lb,ub,x,V,nu,S0,K,1);
    % Solve the linear system with PSOR -> since we need to use an
    % iterative algorithm to impose conditions
    rhs=(M2*V+BC-I);
    maxiter=500; tol=1e-4; w=1.5;     
    Vold=V;
    for k=1:maxiter
        for i=1:N+1
            if i==1
                y=1/M1(i,i)*(rhs(i)-M1(i,i+1)*Vold(i+1));
            elseif i==N+1
                y=1/M1(i,i)*(rhs(i)-M1(i,i-1)*V(i-1));
            else
                y=1/M1(i,i)*(rhs(i)-M1(i,i+1)*Vold(i+1)...
                    -M1(i,i-1)*V(i-1));
            end
            V(i)=max(Vold(i)+w*(y-Vold(i)), K-S0*exp(x(i))); 
            % V(t,S)>=K-S inside the linear system solution
        end
...
v=zeros(size(y));
%% y<=xmin              BOUNDARY CONDITION on xMIN (extend also outside)
index=(y<=x(1));
v(index)=K*disc-S0*exp(y(index));
%% y>= xmax             BOUNDARY CONDITION on xMAX (extend also outside)
%index=(y>=x(end));
%v(index)=0;
%% xmin<y<xmax
index=find((y>x(1)).*(y<x(end)));
v(index)=interp1(x,V,y(index)); % interpolation
end
