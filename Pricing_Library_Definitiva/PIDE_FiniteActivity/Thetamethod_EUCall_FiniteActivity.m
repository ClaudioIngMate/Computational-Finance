...
% At maturity
V=max(S0*exp(x')-K,0); %%%%%%%%%%%%%%%%%%% Payoff at maturity (EU Call)
BC=zeros(N+1,1); %%%%%%%%%%%%%%% Boundary condition on xmin (EU Call)

for j=M:-1:1
    % known t_j --> unknown t_{j-1}
    BC(end)=S0*exp(xmax)-K*exp(-r*(T-(j-1)*dt)); %%%%%%%%%%%%%% Boundary condition xmax
...
index=(y<=x(1));
v(index)=0; %%%%%%%%%%%%%%%% Boundary condition xMIN (EU Call)
%% y>= xmax    BOUNDARY CONDITION on xMAX
index=(y>=x(end));
v(index)=S0*exp(y(index))-K*disc; %%%%%%%%%% Boundary condition xMAX (EU Call)
%% xmin <y <xmax
