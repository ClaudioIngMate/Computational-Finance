%% Knock&Out Put option with MonteCarlo
...
S=
% 2. Discounted Payoff
DiscPayoff=exp(-r*T)*max(K- S(:,end),0).*( max(S,[],2)<U).*( min(S,[],2)>L);
% 3. Compute the Price
...
% 2. Discounted Payoff
DiscPayoff=exp(-r*T)*max(K- S(:,end),0).*( max(S,[],2)<U ).*( min(S,[],2)>L );
DiscPayoffav=exp(-r*T)*max(K- Sav(:,end),0).*( max(Sav,[],2)<U ).*( min(Sav,[],2)>L );
% 3. Compute the Price
...
f=S(:,end);
g=exp(-r*T)*max(K-f,0).*( max(S,[],2)<U ).*( min(S,[],2)>L );
VC=cov(f,g);
alpha=-VC(1,2)/VC(1,1);
% 2. compute the price
