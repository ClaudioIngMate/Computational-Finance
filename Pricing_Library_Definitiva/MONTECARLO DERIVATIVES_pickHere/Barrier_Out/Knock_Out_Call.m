...
S=
% 2. Discounted Payoff
DiscPayoff=exp(-r*T)*max(S(:,end)-K,0).*( max(S,[],2)<U).*( min(S,[],2)>L);
% 3. Compute the Price
...
% 2. Discounted Payoff
DiscPayoff=exp(-r*T)*max(S(:,end)-K,0).*( max(S,[],2)<U ).*( min(S,[],2)>L );
DiscPayoffav=exp(-r*T)*max(Sav(:,end)-K,0).*( max(Sav,[],2)<U ).*( min(Sav,[],2)>L );
% 3. Compute the Price
...
f=S(:,end);
g=exp(-r*T)*max(f-K,0).*( max(S,[],2)<U ).*( min(S,[],2)>L );
VC=cov(f,g);
alpha=-VC(1,2)/VC(1,1);
% 2. compute the price
